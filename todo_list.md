...
### 3.6 NLLB核心翻译功能实现（新增）
- [ ] 3.6.1 NLLB本地服务环境配置与依赖安装（个人电脑部署方案）
- [ ] 3.6.2 NLLB本地服务启动与健康检查验证（支持日1000次翻译）
- [ ] 3.6.3 主服务翻译接口切换到本地NLLB优先模式
- [ ] 3.6.4 翻译服务负载均衡机制验证（本地→云端→Mock→Fallback）
- [ ] 3.6.5 前端翻译功能端到端测试（所有语言对）
- [ ] 3.6.6 文档翻译服务与本地NLLB集成测试
- [ ] 3.6.7 翻译缓存机制优化（减少重复推理）
- [ ] 3.6.8 翻译性能优化（批处理、并发限制、内存管理）
- [ ] 3.6.9 错误处理与降级策略完善
- [ ] 3.6.10 本地翻译服务监控与日志系统
...
### 4.6 NLLB翻译功能测试（新增）
- [ ] 4.6.1 本地NLLB服务单元测试（单文本翻译、批量翻译）
- [ ] 4.6.2 翻译质量评估测试（准确性、一致性、语法正确性）
- [ ] 4.6.3 翻译性能基准测试（响应时间、吞吐量、并发处理）
- [ ] 4.6.4 翻译服务容错测试（服务故障、网络中断、内存不足）
- [ ] 4.6.5 前端翻译界面功能回归测试
- [ ] 4.6.6 文档翻译端到端集成测试
- [ ] 4.6.7 多语言翻译路径完整性测试
- [ ] 4.6.8 翻译缓存机制有效性测试
