...
export function BidirectionalNavigation({
  ...
  // 获取当前语言信息
  const sourceLanguageInfo = APP_CONFIG.languages.supported.find(lang => lang.code === currentSourceLang)
  const targetLanguageInfo = currentTargetLang === 'en' ? 
    APP_CONFIG.languages.target : 
    APP_CONFIG.languages.supported.find(lang => lang.code === currentTargetLang)
  ...
  const getRelatedLanguages = () => {
    return APP_CONFIG.languages.supported
      .filter(lang => lang.code !== currentSourceLang && lang.code !== currentTargetLang)
        ...
        <Link href={reverseUrl} className="group">
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <span className="font-medium">{targetLanguageInfo?.nativeName || 'English'}</span>
              <ArrowRight className="h-3 w-3" />
              <span className="font-medium">{sourceLanguageInfo?.nativeName || 'English'}</span>
            </div>
...
export function useBidirectionalNavigation() {
  ...
  const navigateToLanguagePair = React.useCallback((sourceLang: string, targetLang: string) => {
    let url = ''
    
    if (targetLang === 'en') {
      // 小语种→英文
      const sourceInfo = APP_CONFIG.languages.supported.find(lang => lang.code === sourceLang)
      url = sourceInfo ? `/${sourceInfo.slug}-to-english` : '/text-translate'
    } else if (sourceLang === 'en') {
      // 英文→小语种
      const targetInfo = APP_CONFIG.languages.supported.find(lang => lang.code === targetLang)
  ...
  const getLanguagePairUrl = React.useCallback((sourceLang: string, targetLang: string) => {
    if (targetLang === 'en') {
      const sourceInfo = APP_CONFIG.languages.supported.find(lang => lang.code === sourceLang)
      return sourceInfo ? `/${sourceInfo.slug}-to-english` : '/text-translate'
    } else if (sourceLang === 'en') {
      const targetInfo = APP_CONFIG.languages.supported.find(lang => lang.code === targetLang)
      return targetInfo ? `/english-to-${targetInfo.slug}` : '/text-translate'
    } else {
