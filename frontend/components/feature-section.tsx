...
type FeatureName = 'ai_powered' | 'fast' | 'privacy' | 'languages' | 'tts' | 'documents'

const featureDetails: Record<FeatureName, { icon: React.ElementType }> = {
  ai_powered: { icon: Sparkles },
  fast: { icon: Zap },
  privacy: { icon: Shield },
...
export function FeatureSection({ t }: { t: (key: string) => string }) {
              ...
              >
                <div className="flex items-center mb-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                    {(() => {
                      const Icon = featureDetails[feature].icon;
                      return <Icon className="h-6 w-6 text-primary" aria-hidden="true" />;
                    })()}
                  </div>
                  <h3 className="ml-3 text-lg font-semibold text-gray-900">{t(`features.${feature}.name`)}</h3>
                </div>
